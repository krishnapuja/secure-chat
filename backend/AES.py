'''
    This Code is written for AES algorithm. This algorithm speaks about the Encryption, Decryption and
...
# Assuming xtime is a function that multiplies a number by 2 in a finite field
def xtime(value):
    # Left shift the value by 1
    result = value << 1

    # If the most significant bit is set, perform a finite field reduction
    if (result & 0x100) != 0:
        result ^= 0x1B  # XOR with the irreducible polynomial x^8 + x^4 + x^3 + x + 1

    return result & 0xFF  # Ensure the result is in the range of a byte (8 bits)
...
def unpad(plaintext):
    """
    Removes padding, returning the unpadded text and ensuring the padding was correct.

    Args:
    - plaintext (bytes): The padded plaintext to be unpadded.
    ...
    """
    if not plaintext:
        raise ValueError("Cannot unpad an empty plaintext")
    ...
    # Extract the message and the padding
    message, padding = plaintext[:-padding_len], plaintext[-padding_len:]

    # Check if the padding bytes are correct
    if not all(byte == padding_len for byte in padding):
...
class AES:
    """
    Class responsible for performing AES operations like Encryption and Decryption. We have implemented only the CBC mode
    of AES. We can expand to OFB, PCBC and CFB modes
    ...
    def encrypt_block(self, plaintext):
        """
        This is a helper method which is used to encrypt the given plaintext. This method is called from
        encrypt() to ensure the encryption happens and the ciphertext is returned. This involves 4 steps
        1. add_round_key
        2.

        Args:
        - plaintext (bytes): The plaintext to be unpadded.
    ...
    def encrypt_cbc(self, plaintext, iv):
        """
        Encrypts the given plaintext using the given initialization vector (iv).
        """
    ...
    def decrypt_cbc(self, ciphertext, iv):
        """
        Decrypts the given  ciphertext using the given initialization vector (iv).
        """
...
def decrypt(key, ciphertext, workload=100000):
    """
    Decrypts `ciphertext` with `key` using AES-128
